...
func (fm *filesManager) _closeUnusedFiles(idToSkip fileID) {
...
func (fm *filesManager) createFile(name string, flags int, bfc beforeFileClose) (*file, error) {
	...
	if fm.limit > 0 && fm.opened >= fm.limit {
		fm._closeUnusedFiles(0)
	}
...
// _openFile opens the given file and sets its state to `fileInUse`.
// If the file's handle is not nil or entry state is not `fileClosed`,
// this call will panic.
// This call will possibly cause opened but unused files to be closed if the
// number of open file requests is above the set limit.
func (fm *filesManager) _openFile(file *file) error {
	fm.Lock()
	if fm.closed {
		fm.Unlock()
...
// _doClose closes the file handle, setting it to nil and switching state to `fileClosed`.
// If a `beforeClose` callback was registered on file creation, it is invoked
// before the file handler is actually closed.
// Lock is required on entry.
func (fm *filesManager) _doClose(file *file) error {
	var err error
...
func (fm *filesManager) lockFile(file *file) (bool, error) {
	...
	return false, fm._openFile(file)
}
...
// _trySwitchState attempts to switch an initial state of `fileOpened`
...
// was `fileOpened`.
func (fm *filesManager) _trySwitchState(file *file, newState int32) (bool, error) {
	wasOpened := false
	wasClosed := false
	for i := 0; i < 10000; i++ {
...
// one of these states to `fileRemoved`, or returned an error if the change can't
...
func (fm *filesManager) remove(file *file) bool {
	wasOpened, err := fm._trySwitchState(file, fileRemoved)
	if err != nil {
		return false
	}
	fm.Lock()
...
func (fm *filesManager) close() error {
	...
	for _, file := range files {
		wasOpened, sserr := fm._trySwitchState(file, fmClosed)
		if sserr != nil {
			if err == nil {
				err = sserr
			}
			...
			fm.Lock()
			if cerr := fm._doClose(file); cerr != nil && err == nil {
				err = cerr
			}
			fm.Unlock()
		}
...
func (ms *FileMsgStore) Store(data []byte) (uint64, error) {
	...
	if fslice == nil || ms.slHasLimits {
		if fslice == nil ||
			...
			// Create new slice
			datFName := filepath.Join(ms.channel, fmt.Sprintf("%s%v%s", msgFilesPrefix, newSliceSeq, datSuffix))
			idxFName := filepath.Join(ms.channel, fmt.Sprintf("%s%v%s", msgFilesPrefix, newSliceSeq, idxSuffix))
